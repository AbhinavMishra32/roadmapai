...
function CustomNode({ data, isConnectable }: {
  ...
  return (
    // <div className={`w-[${ data.isExpanded ? 240 : 220}px] h-fit bg-white transition-all duration-300`}>
      <div
        className={`group relative px-4 py-3 shadow-md dark:shadow-[0_5px_60px_-15px_rgba(154,157,241,0.2)] rounded-lg border transition-all duration-300 hover:shadow-lg hover:scale-105 ${data.isHighlighted ? 'border-red-400 dark:border-indigo-500 bg-red-50 dark:bg-indigo-700/10 backdrop-blur-sm' : 'border-gray-200 dark:border-neutral-700 dark:border-2 bg-white dark:bg-neutral-900/10 backdrop-blur-sm'
          ...
          {data.nextSteps && (
            <div className="flex gap-1 flex-wrap mt-1">
              {data.nextSteps.map((step, index) => (
                <span
                  key={index}
                  className={`px-2 py-0.5 rounded-full text-xs font-medium dark:border-indigo-900 dark:border-[1px] backdrop-blur-3xl ${data.isHighlighted ? 'bg-red-100 dark:bg-indigo-900/50 text-red-800 dark:text-gray-200' : 'bg-yellow-100 dark:bg-indigo-900/30 text-yellow-800 dark:text-gray-300'
                    }`}
                >
                  {step}
                </span>
