...
export default function CareerUnsure() {
  ...
  const handleQuizComplete = async () => {
          ...
          alternativeCareers: {
            ...
            items: {
              ...
              properties: {
                career: { type: SchemaType.STRING },
                description: { type: SchemaType.STRING },
                potentialOutcomes: { type: SchemaType.ARRAY, items: { type: SchemaType.STRING } }
              }
            }
          },
          danceCareerAnalysis: {
            type: SchemaType.OBJECT,
            properties: {
              similarCareersWithHigherPay: { type: SchemaType.ARRAY, items: { type: SchemaType.STRING } },
              whyChooseDance: { type: SchemaType.ARRAY, items: { type: SchemaType.STRING } },
              whyNotChooseDance: { type: SchemaType.ARRAY, items: { type: SchemaType.STRING } }
            }
          },
          finalRecommendation: { type: SchemaType.STRING }
        },
        required: ["careers", "overallAnalysis", "alternativeCareers", "danceCareerAnalysis", "finalRecommendation"]
      };
    ...
    const prompt = `
      ...
      provide Generate a comprehensive career analysis for a person considering a career in dance. 
  Include detailed information about the dance career, alternative careers with potential outcomes, 
  analysis of dance careers including similar careers with higher pay, reasons for and against 
  choosing a dance career, and a final recommendation. The analysis should be thorough and 
  cover all aspects defined in the schema, including metrics, pros and cons, and personalized advice. The salary range should be specific to India
    `
    // try {
    //   const result = await model.generateContent(prompt)
